...
The integers $q$ and $r$ are called the _quotient_ and the _remainder_, respectively, of the division of $a$ by $d$. The next section is dedicated to the special case when $r=0$; here we denote $q$ by the symbol $\frac ad$.

```python
# Most programming languages have built-in methods to compute these
# For example Python uses the following:
...
>
>If, on the other hand, $d\leq a$, then applying the inductive hypothesis to the natural $a-d$, which is of course less than $a$, we get $q',r$ integers such that $a-d=dq'+r$. But then, $a=d(q'+1)+r$ so letting $q=q'+1$ I am done.

This is another illustrative example of how strong induction can be useful. Attempting to prove this be weak induction, one runs into the problem of having to use the fact that $a$ can be written as $a=dq+r$ to accomplish the same for $a+1$, which is cumbersome and not intuitive.
...
>[!def] Definition.
>
>Let $a$ and $b$ be integers. We say $a$ _divides_ $b$, in symbols $a\mid b$, if there is an integer $s$ such that $b=sa$. Under these circumstances, we also say that $a$ is a _divisor_ or _factor_ of $b$, and that $b$ is a _multiple_ of $a$.

>[!question] Reflect.
>
>How are this definition and the Q-R formula related? Could we have defined the relation $\mid$ in terms of the quotient and remainder? How?
...
Let us prove some elementary properties of divisibility before we leave some exercises.
...
>
>1. Every integer divides itself. In symbols, $a\mid a$. (Reflexivity)
>2. If $a\mid b$ and $b\mid c$, then $a\mid c$. (Transitivity)
>3. If $c\mid a$ and $c\mid b$, then for any $s,t\in\mathbb Z$, $c\mid sa+tb$. (Linear combinations)

>[!proof] Proof:
>
>1. Since $n=1\cdot n$, $n\mid n$. The same equality proves another fact as well.
...
>[!note] Exercises.
>
>1. For all $n\in\mathbb Z$, $1\vert n$ and $n\vert 0$.
>2. If $n$ is even, then $4\vert n^2$; but if $n$ is odd, then $8\vert n^2-1$.
...
>>[!hint]- Hint.
>>
>>Use that ${n\choose k}={n-1\choose k-1}+{n-1\choose k}$ and apply induction. Alternatively, use the result proved in Tutorial 3.
...
>[!note] Exercise.
>
>Show that if an integer $n>1$ has no divisors $k$ satisfying $2\leq k\leq\sqrt n$, then $n$ must be prime. Give an example as to why the bound on $k$ is the best possible. In other words, the statement fails when we replace the bound by $2<k\leq\sqrt n$ or $2\leq k<\sqrt n$.
